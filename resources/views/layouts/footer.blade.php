<footer class="bg-navy-dark text-white">
    <div class="max-w-7xl mx-auto px-6 py-12">
        <div class="grid grid-cols-1 md:grid-cols-5 gap-8">
            {{-- Logo & Company --}}
            <div class="md:col-span-1">
                <div class="flex items-center gap-3 mb-4">
                    <img src="{{ asset('images/logo.png') }}" alt="Meerkats Logo" class="h-10 w-auto">
                </div>
                <p class="text-sm font-semibold leading-snug">
                    PT Meerkats<br>Flexipack Indonesia
                </p>
            </div>

            {{-- Navigation --}}
            <div>
                <h4 class="font-bold text-sm uppercase tracking-wider mb-4">Navigation</h4>
                <ul class="space-y-2 text-sm text-white/70">
                    <li><a href="{{ route('welcome') }}" class="hover:text-white transition-colors">Home</a></li>
                    <li><a href="{{ route('about') }}" class="hover:text-white transition-colors">About</a></li>
                    <li><a href="{{ route('products') }}" class="hover:text-white transition-colors">Products</a></li>
                    <li><a href="{{ route('machinery') }}" class="hover:text-white transition-colors">Machinery</a></li>
                    <li><a href="{{ route('contact') }}" class="hover:text-white transition-colors">Contact</a></li>
                </ul>
            </div>

            {{-- Location --}}
            <div>
                <h4 class="font-bold text-sm uppercase tracking-wider mb-4">Location</h4>
                <p class="text-sm text-white/70 leading-relaxed">
                    Jalan Pemda Tigaraksa,<br>
                    Kab. Tangerang, Banten<br>
                    15720, Indonesia
                </p>
            </div>

            {{-- Contact Us --}}
            <div>
                <h4 class="font-bold text-sm uppercase tracking-wider mb-4">Contact Us</h4>
                <ul class="space-y-2 text-sm text-white/70">
                    <li>HP: 081-2222</li>
                    <li>Tel: 021-3333</li>
                    <li>info@meerkatsflexipack.co.id</li>
                </ul>
            </div>

            {{-- Social Media --}}
            <div>
                <h4 class="font-bold text-sm uppercase tracking-wider mb-4">Social Media</h4>
                <ul class="space-y-2 text-sm text-white/70">
                    <li class="flex items-center gap-2">
                        <svg class="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
                        <span>meerkats_flexipack</span>
                    </li>
                </ul>
            </div>
        </div>
    </div>

    {{-- Copyright --}}
    <div class="border-t border-white/10">
        <div class="max-w-7xl mx-auto px-6 py-4">
            <p class="text-center text-xs text-white/50">
                &copy; {{ date('Y') }} Meerkats Flexipack Indonesia. All Rights Reserved.
            </p>
        </div>
    </div>
</footer>